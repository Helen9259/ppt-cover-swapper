...
설치 여부와 무관하게 정확히 렌더링됩니다 (`src/lib/pptxCapture.ts`의 `NAMED_BUNDLED_FONTS`에 필요한
폰트를 추가할 수 있습니다).

그 외의 폰트를 참조하는 PPT를 업로드하면 "1. PPTX 파일 업로드" 아래에 어떤 폰트를 찾지 못했는지
표시되고, 폰트 파일(.ttf/.otf/.woff)을 가지고 있다면 그 자리에서 업로드해 즉시 정확한 폰트로 다시
캡처할 수 있습니다. 브라우저 권한이 필요 없는 일반 파일 선택이며, 업로드한 폰트는 세션 동안
같은 폰트를 쓰는 다른 파일에도 자동으로 적용됩니다. 어떤 폰트가 빠졌는지는 pptx-glimpse의 변환
리포트(`diagnostics`)에서 `font.notFound` 경고를 직접 읽어 판단합니다 — PPT 파일 자체에 폰트가
내장되어 있는지 여부와 무관하게, 렌더링 시점에 실제로 못 찾은 폰트만 정확히 짚어냅니다.
